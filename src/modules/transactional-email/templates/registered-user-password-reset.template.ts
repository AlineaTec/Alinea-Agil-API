...
export function renderIdentityRegisteredUserPasswordReset(params: {
...
<p>Recibimos una solicitud para restablecer la contraseña de tu cuenta en <strong>${BRAND_PRODUCT_LINE}</strong>.</p>
...
<p><small>Este enlace caduca en breve. Si no solicitaste el cambio, ignora este mensaje; tu contraseña no se modifica.</small></p>`
  ...
  const text = [
    `Hola ${name},`,
    ``,
    `Restablece tu contraseña de ${BRAND_PRODUCT_LINE} abriendo:`,
    link,
    ``,
  ...
  return {
    subject: `Restablecer contraseña — ${BRAND_PRODUCT_LINE}`,
